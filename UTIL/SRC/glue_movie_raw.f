...
!--------------------
	open(1,file=trim(filepath)
     & //'.info.movie',form='formatted',status='old')
	...
	nchunks = max(1_8,1_8*nx*nsubdomains_x
     & *ny*nsubdomains_y*thetimes/1000000000_8)
	print*,'nchunks=',nchunks
	chunk = int(thetimes/nchunks)
	print*, 'chunk=',chunk
	allocate(tmpdata(nx,ny,chunk), 
     & bigdata(nx*nsubdomains_x,ny*nsubdomains_y,chunk))
	...
	do i=1,9  !numtits
     ...
     &             recl=(nx*nsubdomains_x)
     &     *(ny*nsubdomains_y)*chunk/bytes_in_rec())

...
#SBATCH --signal=B:USR1@200 #sig_time (300 secs) should match your checkpoint overhead time
...
function combine_and_move {
    file_name1=`echo $1 | sed 's/com3D_0/com3D/'`
    echo "*** file $file_name1 ***"
    $UTIL_DIR/com3D2nc_sep $file_name1 &&  mv "$file_name1"_* uncombined_files
}
...
get_complete_file_size() {
	...
	first_filename=$(find . -maxdepth 1 -name '*com3D_0' -print -quit)
	files_to_combine=`echo $first_filename | sed 's/com3D_0/com3D/'`
	nc_file=`echo $first_filename | sed 's/com3D_0/nc/'`
	$UTIL_DIR/com3D2nc_sep $files_to_combine && successful_file_size="$(wc -c <"$nc_file")"
...
find . -maxdepth 1 -name '*com3D_0' -print0 | xargs --replace=@ -0 -P $SLURM_NTASKS  bash -c 'combine_and_move $"@"' _ &
